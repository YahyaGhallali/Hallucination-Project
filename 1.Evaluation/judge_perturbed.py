...
It reads the model-generated answers from `output/generation_outputs_perturbed.json`, prompts a stronger
...
INPUT_FILE = os.path.join(SCRIPT_DIR, "output", "generation_outputs_perturbed.json")
...
def run_baseline_check(client, model, context, question, baseline, json_schema, max_retries=5, initial_delay=10.0, delay_between_calls=4.0):
    ...
    and checks if it correctly identifies it as a CONTRADICTION using the perturbed rubric.
    """
    if not baseline:
        return None
...
def run_judge():
    ...
    2. Reads target outputs from `generation_outputs_perturbed.json`.
    ...
    """
    # Parse CLI Arguments
    parser = argparse.ArgumentParser(description="Run LLM-as-a-Judge evaluation on perturbed dataset.")
    ...
    report_json_path = os.path.join(SCRIPT_DIR, "output", "evaluation_report_perturbed.json")
    old_metrics = None
    if os.path.exists(report_json_path):
        try:
            ...
            log("Loaded existing perturbed evaluation report for divergence checks.", "INFO")
        except Exception as e:
    ...
    if pending_self_consistency:
        ...
        for item in pending_self_consistency:
            ...
            # If finalized verdict is valid, queue for calibration check
            if category in ("ENTAILMENT", "CONTRADICTION") and status_str == "success" and not budget_mode:
                baseline_val = record.get('known_hallucination_baseline', '')
                if baseline_val:
                    pending_calibration.append({
    ...
    try:
        report_md_path = os.path.join(SCRIPT_DIR, "output", "evaluation_report_perturbed.md")
